...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    try:
        ...
        bot_reply = completion.choices[0].message.content
                ...
                # Умный перехват и страховка на случай пустого ответа
        user_text_lower = user_input.lower()
        
        # 1. Если медпред назвал цифры/аргументы — сдаемся!
        ...
        # 2. Если модель выдала пустой ответ — даем естественную реакцию по смыслу
        elif not bot_reply or not bot_reply.strip():
            if any(w in user_text_lower for w in ["утро", "чувству", "мама", "ребенок", "ноч"]):
                bot_reply = "Утром они оба измотаны: ребенок кашляет до рвоты и одышки, а мама не спала всю ночь."
            elif any(w in user_text_lower for w in ["задержк", "мокрот", "кашель", "быва"]):
                bot_reply = "Да, задержка мокроты бывает часто, из-за этого кашель только усиливается."
            else:
                bot_reply = "Я привыкла к своему препарату. Назовите конкретные цифры и факты, чем ваш лучше?"
