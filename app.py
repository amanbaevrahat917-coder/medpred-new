...
# === СЦЕНАРИИ С ЧЕТКИМИ ТРИГГЕРАМИ "СДАЧИ" ===
SPIN_SCENARIOS = {
    "1. Врач только отпаивает (Ничего не назначает)": {
        "doctor_profile": "Ты придерживаешься строго физиологичного подхода. Сначала говори: 'Я не рекомендую сиропы от кашля, только обильное питьё'.",
    ...
    "2. Назначает конкурента: МУНАТОРИЛ": {
        "doctor_profile": "Ты изначально назначаешь Мунаторил. Сначала говори: 'Мунаторил'.",
    ...
    "3. Назначает конкурента: ГЕРБИОН": {
        "doctor_profile": "Ты изначально назначаешь Гербион. Сначала говори: 'Гербион'.",
    ...
    "4. Назначает конкурентов: ПРОСПАН / ЛИНКАС (Горькие)": {
        "doctor_profile": "Ты изначально назначаешь Проспан (или Бронхипрет/Линкас). Сначала говори: 'Проспан'.",
...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    # === УМНЫЙ ПРОМПТ: ОБУЧЕНИЕ МЕДПРЕДОВ ===
    system_prompt = f"""
    ...
    ПРАВИЛА ТРЕНАЖЕРА:
    1. Не здоровайся повторно! Отвечай коротко (1-2 предложения).
    2. Подыгрывай логике SPIN: если МП задает правильный усугубляющий вопрос, отвечай по шпаргалке (про ночной кашель, одышку и т.д.).
    3. АДЕКВАТНАЯ РЕАКЦИЯ НА ФАКТЫ: Ты не должна сдаваться просто так. Если МП просто просит "назначьте наш препарат" без аргументов — откажи и скажи "Я доверяю проверенным средствам".
    4. НО если МП назвал РЕАЛЬНЫЕ ЦИФРЫ И ФАКТЫ из условия твоей капитуляции (например, сравнил состав, сказал про дозировку "в 9 раз больше" или про приятный вкус) — ты ДОЛЖНА признать его правоту. Восхитись аргументом и скажи, что теперь готова назначать его препарат.
    """
