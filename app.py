...
# === ИСПРАВЛЕННЫЕ СЦЕНАРИИ (Без привязки к "первому" сообщению) ===
SPIN_SCENARIOS = {
    ...
    "2. Назначает конкурента: МУНАТОРИЛ": {
        "doctor_profile": "Твоя позиция: ты всегда назначаешь сироп Мунаторил. Если тебя спрашивают, какой сироп ты рекомендуешь деткам — уверенно отвечай: 'Обычно я назначаю Мунаторил'.",
    ...
    "3. Назначает конкурента: ГЕРБИОН": {
        "doctor_profile": "Твоя позиция: ты всегда назначаешь Гербион. Если тебя спрашивают, какой сироп ты рекомендуешь — уверенно отвечай: 'Из сиропов я предпочитаю назначать Гербион'.",
...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    with st.chat_message("Медицинский представитель", avatar="💼"):
        st.write(user_input)
          ...
          # === НАДЕЖНЫЙ РОЛЕВОЙ ПРОМПТ ===
    system_prompt = f"""
    Ты — врач-педиатр Асель Аскаровна. Ты находишься на приеме, к тебе пришел медицинский представитель компании SELTFAR.
    
    КОНТЕКСТ И ТВОЯ ПОЗИЦИЯ:
    {scenario_data['doctor_profile']}
    
    КАК ТЕБЕ ОТВЕЧАТЬ НА ВОПРОСЫ:
    - На вопросы про задержку мокроты, симптомы или осложнения отвечай реалистично с точки зрения врача (да, кашель усиливается по ночам, бывает одышка, обструкция, приходится назначать гормоны).
    - Если медпред просто просит "начните назначать наш препарат" без конкретных фактов — вежливо отказывай ("Я привыкла работать со проверенными средствами").
    - УСЛОВИЕ СОГЛАСИЯ: Соглашайся переключиться на препарат SELTFAR ТОЛЬКО если медпред назвал конкретные факты: {scenario_data['surrender_trigger']}.
    
    ПРАВИЛА ОБЩЕНИЯ:
    1. Отвечай коротко и естественно (1–2 предложения), как реальный врач на рабочем месте.
    2. Не здоровайся повторно, если диалог уже идет.
    3. Никогда не молчи и не пиши служебных фраз.
    """
