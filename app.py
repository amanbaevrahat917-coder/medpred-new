...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    1. На вопросы о том, что ты обычно назначаешь — говори свою стартовую позицию.
    2. На вопросы про симптомы, ночной кашель и выматывание мамы с ребенком — отвечай развернуто от себя, подтверждая проблему (например: "Да, мама с ребенком измотаны, ребенок кашляет всю ночь, наутро оба без сил").
    3. Если медпред говорит общие слова без конкретных цифр — отказывай: "Я привыкла к своему препарату. Чем ваш лучше в цифрах?"
    4. ГЛАВНОЕ ПРАВИЛО (КАПИТУЛЯЦИЯ): Если медпред назвал факты/цифры: [{scenario_data['surrender_trigger']}] — ты ДОЛЖНА удивленно сдаться: "Ничего себе разница в дозировке! Это серьезный аргумент, я готова попробовать ваш сироп."
    
    Отвечай живым языком, строго 1-2 предложениями, не повторяй одинаковые фразы!
    """
    ...
    try:
        completion = client.chat.completions.create(
            ...
            temperature=0.6, # Чуть подняли температуру для разнообразия речи
            max_tokens=200
        )
        ...
        # Страховка 1: Перехват цифр и ключевых слов
        ...
        # Страховка 2: Заглушка, если модель зависла или выдала пустую строку
        elif not bot_reply or not bot_reply.strip():
            if any(w in user_text_lower for w in ["утро", "чувству", "мама", "ребенок", "состоян"]):
                bot_reply = "Утром они оба абсолютно обессилены: ребенок кашляет до рвоты, а мама не спала всю ночь."
            elif any(w in user_text_lower for w in ["задержк", "мокрот", "кашель", "быва"]):
                bot_reply = "Да, задержка мокроты действительно бывает, из-за этого кашель становится только тяжелее."
            else:
