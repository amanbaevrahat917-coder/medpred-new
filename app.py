...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    ПРАВИЛА ТРЕНАЖЕРА:
    1. На первое сообщение пользователя всегда отвечай так, как указано в твоем контексте (с приветствием). Но в следующих сообщениях НЕ здоровайся повторно!
    2. Отвечай коротко (1-2 предложения).
    3. Подыгрывай логике SPIN: если МП задает правильный усугубляющий вопрос, отвечай по шпаргалке.
    ...
    5. НО если МП назвал РЕАЛЬНЫЕ ЦИФРЫ И ФАКТЫ из условия твоей капитуляции — ты ДОЛЖНА признать его правоту и согласиться.
    """
    ...
    try:
        ...
        # Заглушка на случай пустого ответа стала логичнее:
