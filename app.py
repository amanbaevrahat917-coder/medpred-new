...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    #     # 1. ПЕРВАЯ ПРИОРИТЕТНАЯ ПРОВЕРКА: ЗАЩИТА ОТ РАННЕЙ ПРОДАЖИ И КАПИТУЛЯЦИЯ
    if selected_scenario_name.startswith("1") and any(w in user_text_lower for w in ["защит", "препарат", "вывод", "авирутус", "сироп", "растительн"]):
        if user_msgs_count >= 4:
            ...
            bot_reply = "Здравствуйте! Вы сразу предлагаете свой препарат, но меня устраивает моя тактика с обильным питьем. Зачем мне лишний раз давать детям сиропы?"
        ...
        if user_msgs_count >= 4:
            ...
            bot_reply = "Здравствуйте! Интересные цифры, но я обычно рекомендую сироп Мунаторил и меня он вполне устраивает. С чего вдруг мне менять препарат?"
        ...
        if user_msgs_count >= 4:
            ...
            bot_reply = "Здравствуйте! Цифры — это хорошо, но я предпочитаю назначать Гербион. Не вижу веских причин переходить на что-то новое."
        ...
        if user_msgs_count >= 4:
            ...
            bot_reply = "Здравствуйте! Я обычно рекомендую Проспан или Линкас. Вкус — не самое главное, они отлично лечат кашель."

    # 2. ОБЫЧНЫЙ СТАРТ ДИАЛОГА (ЕСЛИ НЕТ РАННЕЙ ПРОДАЖИ)
    elif user_msgs_count == 1 and not is_asking_prescription:
        bot_reply = "Здравствуйте! Проходите, присаживайтесь. Слушаю вас."
