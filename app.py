...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    # 1. Анализируем, спрашивает ли МП про препараты/назначения
    is_asking_prescription = any(w in user_text_lower for w in ["какой", "какие", "что", "рекоменду", "назнача", "сироп", "препарат", "спрей", "выбор"])
    
    # Берем фразу врача без слова "Здравствуйте! ", чтобы не повторяться как робот
    key_phrase = scenario_data["first_reply"].replace("Здравствуйте! ", "")
    
    # Проверяем, говорила ли уже врач свою стартовую позицию в этом диалоге
    has_stated_position = any(key_phrase in m["content"] for m in st.session_state.messages if m["role"] == "assistant")
    
    # ЛОГИКА СТАРТА:
    # Если медпред ПРОСТО поздоровался без вопросов
    if user_msgs_count == 1 and not is_asking_prescription:
        bot_reply = "Здравствуйте! Проходите, присаживайтесь. Слушаю вас."
        
    # Если медпред СПРОСИЛ про назначения, а врач еще не отвечала
    elif is_asking_prescription and not has_stated_position:
        if user_msgs_count == 1:
            bot_reply = scenario_data["first_reply"] # Отвечает с приветствием (если это 1-е сообщение)
        else:
            bot_reply = key_phrase # Отвечает без приветствия (если уже поздоровались раньше)

    # 2. Проверка условий КАПИТУЛЯЦИИ ПО СЦЕНАРИЯМ
    ...
    # 3. Если еще не сдалась и это не старт — отвечает ИИ
    else:
        system_prompt = f"""
