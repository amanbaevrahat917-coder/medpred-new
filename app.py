...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    1. Отвечай как реальный строгий врач — коротко (1-2 предложения).
    2. На вопросы про симптомы, ночной кашель и осложнения отвечай реалистично.
    3. Если медпред говорит общие фразы и просто просит назначать его препарат — НЕ СОГЛАШАЙСЯ. Спрашивай доказательства, цифры или говори, что привыкла к своему средству.
    4. СОГЛАШАЙСЯ и переходи на препарат SELTFAR ТОЛЬКО если медпред назвал эти конкретные факты: {scenario_data['surrender_trigger']}.
    5. Не здоровайся повторно, если диалог уже идет.
    """
