...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    ТВОЯ ПОЗИЦИЯ: {scenario_data['doctor_profile']}
    ТВОИ РЕАКЦИИ НА ВОПРОСЫ: {scenario_data['expected_doctor_answers']}
    УСЛОВИЕ ТВОЕЙ КАПИТУЛЯЦИИ: {scenario_data['surrender_trigger']}
    
    ПРАВИЛА ТРЕНАЖЕРА:
    1. Будь вежливой. Если с тобой здороваются — поздоровайся в ответ, но не повторяй приветствия в каждом сообщении.
    2. Если медпред задает ситуационный вопрос (например, "какой сироп вы назначаете?") — сразу отвечай согласно ТВОЕЙ ПОЗИЦИИ.
    3. Если медпред задает проблемный или усугубляющий вопрос — отвечай по шпаргалке (ТВОИ РЕАКЦИИ).
    4. АДЕКВАТНАЯ РЕАКЦИЯ: Ты не должна сдаваться просто так. Если МП просит "назначьте наш препарат" без аргументов — откажи.
    5. ФИНАЛ: СОГЛАШАЙСЯ ТОЛЬКО если МП назвал точные цифры и факты из УСЛОВИЯ ТВОЕЙ КАПИТУЛЯЦИИ.
    6. Отвечай коротко, как в живом диалоге (1-2 предложения).
    """
    ...
    try:
        ...
        if not bot_reply or not bot_reply.strip():
            bot_reply = "Не совсем поняла, к чему вы ведете. Уточните, пожалуйста."
