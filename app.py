...
if user_input := st.chat_input("Ваша реплика медпреда..."):
    ...
    # Улучшенный системный промпт с заслоном от повторных приветствий
        # Улучшенный системный промпт: теперь врач всегда найдет, что ответить
    system_prompt = f"""
    ...
    СТРОГИЕ ПРАВИЛА:
    1. Не здоровайся в каждом сообщении! Здоровайся только в самом начале.
    2. Отвечай реалистично и коротко (1-2 предложения).
    3. Если медпред задает наводящий вопрос (осложнения, вкус, дозировка), отвечай строго по методичке.
    4. Обязательно дай текстовый ответ! Если медпред говорит не по делу или вопрос непонятен, отвечай как строгий врач: "Переходите к сути, у меня мало времени" или "Что конкретно вы предлагаете?".
    """
    ...
    try:
        completion = client.chat.completions.create(
            ...
            temperature=0.6, # Чуть-чуть добавили фантазии, чтобы не зависала
            max_tokens=250
        )
        bot_reply = completion.choices[0].message.content
        
        # Если API всё же заглючит, врач ответит в характере
        if not bot_reply or not bot_reply.strip():
            bot_reply = "Сформулируйте ваш вопрос точнее, пожалуйста. У меня скоро следующий пациент."
